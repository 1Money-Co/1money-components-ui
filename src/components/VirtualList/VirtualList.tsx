...
export function RawList<T>(props: ListProps<T>, ref: React.Ref<ListRef>) {
  const {
    prefixCls = VIRTUAL_LIST_PREFIXCLS,
    className,
    height,
    itemHeight,
    fullHeight = true,
  ...
  const diffItemRef = useRef<T | null>(null);
  ...
  let componentStyle: React.CSSProperties | undefined = undefined;
  if (height) {
...
const VirtualListBase = React.forwardRef<ListRef, ListProps<any>>(RawList);

VirtualListBase.displayName = VIRTUAL_LIST_COMPONENT_NAME;

const VirtualList = React.memo(VirtualListBase) as <Item = any>(
