...
const SummaryCell: React.FC<React.PropsWithChildren<SummaryCellProps>> = props => {
  ...
  const mergedStickyOffsets = stickyOffsets ?? { start: [], end: [], widths: [], isSticky: false };
  const mergedFlattenColumns = flattenColumns ?? [];
  const lastIndex = index + colSpan - 1;
  const mergedColSpan = lastIndex + 1 === scrollColumnIndex ? colSpan + 1 : colSpan;

  const fixedInfo = React.useMemo(
    () =>
      getCellFixedInfo(index, index + mergedColSpan - 1, mergedFlattenColumns, mergedStickyOffsets),
    [index, mergedColSpan, mergedFlattenColumns, mergedStickyOffsets],
  );
